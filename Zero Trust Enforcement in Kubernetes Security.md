...
### **Phase 8: Platform, Human, and Operator Security**

*Objective: Secure the underlying platform, human access paths, and the operators that enforce Zero Trust.*

* **The Problem:** Zero Trust often overlooks the Node OS, human administrators, and cluster operators that may hold cluster-admin roles.
...
* **The Fix \- Operator RBAC:** Audit and restrict Operator permissions.
...
* **Long-term signal:** Automate CIS scans, OIDC/JIT access audits, and operator RBAC diff reports; require signed, reviewed PRs for operator upgrades and add Falco rules for operator namespaces so escalations are detected quickly.
